	...
	function nameSuccessFunction(e){
		layer.confirm('确定删除该分类？删除之后该分类下所有文章会自动移动到“心情随笔”分类下。', {
		  btn: ['确定','取消'] //按钮
		}, function(){
			// 打包json数据格式
			var date_json = {cate_id:$(e).attr('cate_id')};
		    // 调用ajax请求
		    commonAjax('POST',deleteCategoryController,openAlbumAjaxSuccessFunction,date_json,'json');
		});
		// 结束函数
		return false;
